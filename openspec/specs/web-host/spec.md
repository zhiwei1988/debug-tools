...
The web-host capability defines the single Node.js process that serves the entire debug-tools web surface — static HTML/JS/CSS assets, the JSON API under `/api/*`, and the upload WebSocket at `/ws` — on one HTTP port with same-origin semantics. It owns the default port and the static-file mount point.
